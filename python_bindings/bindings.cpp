...
class Index {
 public:
    ...
    py::object knnQuery_return_numpy(
        ...
        long long count=0;
        std::vector<std::vector<std::pair<dist_t, hnswlib::labeltype>>> all_results;
        if (num_threads <= 0)
            num_threads = num_threads_default;

        {
            ...
            if (normalize == false) {
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    ...
                    std::priority_queue<std::pair<float, hnswlib::labeltype >> result = appr_alg->searchwithDistance(
                        (void*)items.data(row), k, 1);
                       // std::cout<<"begin4"<<std::endl;
                     int num=result.size();
                     std::vector<std::pair<dist_t, hnswlib::labeltype>> result_vec;
                     for (int i = num - 1; i >= 0; i--) {
                        auto& result_tuple = result.top();
                      
                        result_vec.push_back(result_tuple);
                        result.pop();
                     }
                ...
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    ...
                    std::priority_queue<std::pair<dist_t, hnswlib::labeltype >> result = appr_alg->searchwithDistance(
                        (void*)(norm_array.data() + start_idx), k, 1);
                    //std::cout<<"begin4"<<std::endl;
                     int num=result.size();
                     std::vector<std::pair<dist_t, hnswlib::labeltype>> result_vec;
                     for (int i = num - 1; i >= 0; i--) {
        ...
        // // 获取总行数和最大列数
        //     int mm=50000;
        //      rows = all_results.size();
        //     size_t max_columns = 0;
        //     std::cout<<max_columns<<"skr"<<std::endl;
        //     for (const auto &result_vec : all_results) {
        //         max_columns = std::max(max_columns, result_vec.size());
        //         //if(max_columns>10000)max_columns=10000;
        //     }
        //     std::cout<<max_columns<<"skr"<<std::endl;
        //     // 创建存储结果的数组
        //     auto data_numpy_l = std::make_unique<hnswlib::labeltype[]>(rows * max_columns);
        //     auto data_numpy_d = std::make_unique<dist_t[]>(rows * max_columns);
        //     size_t row_idx = 0;
            
        //     // 将结果填充到数组中
        //     for (const auto &result_vec : all_results) {
        //         size_t col_idx = max_columns - result_vec.size();
        //         for (const auto &result_tuple : result_vec) {
        //             data_numpy_l[row_idx * max_columns + col_idx] = result_tuple.second;
        //             data_numpy_d[row_idx * max_columns + col_idx] = result_tuple.first;
        //             //std::cout<<result_tuple.first<<"   "<<result_tuple.second<<" ; ";
        //             col_idx++;
        //         }
        //         row_idx++;
        //         //std::cout<<std::endl;
        //        // std::cout<<result_vec[1].first<<"   "<<result_vec[1].second<<" ; "<<std::endl;
        //     }


        // return py::make_tuple(
        //     py::array_t<hnswlib::labeltype>({rows, max_columns}, {max_columns * sizeof(hnswlib::labeltype), sizeof(hnswlib::labeltype)}, reinterpret_cast<hnswlib::labeltype *>(data_numpy_l.get())),
        //     py::array_t<dist_t>({rows, max_columns}, {max_columns * sizeof(dist_t), sizeof(dist_t)}, reinterpret_cast<dist_t *>(data_numpy_d.get()))
        // );
            std::vector<std::vector<std::pair<dist_t, hnswlib::labeltype>>> result_vectors;

            // 将结果填充到数组中
            for (const auto &result_vec : all_results) {
                std::vector<std::pair<dist_t, hnswlib::labeltype>> result_vector;
                for (const auto &result_tuple : result_vec) {
                    result_vector.push_back(result_tuple);
                }
                result_vectors.push_back(result_vector);
            }

            // 转换为 Python 对象
            py::list result_list;
            for (const auto &result_vec : result_vectors) {
                py::list result_sublist;
                for (const auto &result_tuple : result_vec) {
                    result_sublist.append(py::make_tuple(result_tuple.second , result_tuple.first));
                }
...
class BFIndex {
 public:
    ...
    //             for (int i = k - 1; i >= 0; i--) {
    //                 auto& result_tuple = result.top();
    //                 data_numpy_d[row * k + i] = std::round(result_tuple.first * 10000.0) / 10000.0;
    ...
    // }
    {
...

        {
            ...
            if (normalize == false) {
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    //std::cout<<"begin3"<<std::endl;
                    std::priority_queue<std::pair<float, hnswlib::labeltype >> result = alg->searchwithDistance(
                ...
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    ...
                    std::priority_queue<std::pair<dist_t, hnswlib::labeltype >> result = alg->searchwithDistance(
