...
class BruteforceSearch : public AlgorithmInterface<dist_t> {
 public:
    ...
    /// @return 
    std::priority_queue<std::pair<dist_t, labeltype >>
    searchWithDistance(const void *query_data, dist_t now_dist , BaseFilterFunctor* isIdAllowed = nullptr) const {
    ...
    std::priority_queue<std::pair<dist_t, labeltype >>
    searchwithDistance(const void *query_data, dist_t dis_threshold) const {
        
        std::priority_queue<std::pair<dist_t, labeltype >> Results;
        if (cur_element_count == 0) return Results;
